...
class tools:
    ...
    def find_unitcell(self):
        ...
        for direction in range(2):
            ...
            self.log[direction]['unique'] = np.unique([atoms[atom_index]['pos'][direction] for atom_index in atoms])
            for layer_index, position in enumerate(self.log[direction]['unique']):
                self.log[direction][layer_index] = {sub_index: item for sub_index, item in atoms.items() if atoms[sub_index]['pos'][direction] == position}

            subcell = {} #here subsctructure will be written 
            for layer_index in range(0, len(self.log[direction]['unique'])): # iterates through layers
                # checks if two layers match
                tmp = self.check_if_layers_match(self.log[direction][layer_index], self.log[direction][0], direction)
        
                if tmp: # if match checks all other layers
                    print("it's a match", layer_index)
                    ...
                    if layer_index - 1 < len(self.log[direction]['unique'])/2:
                        print("translation is possible", layer_index,len(self.log[direction]['unique'])/2)
                        for j in range(1, layer_index):
                            #checking other layers
                            check_all = self.check_if_layers_match(self.log[direction][layer_index+j], self.log[direction][j], direction)
                            if not check_all:
    ...
    def check_if_layers_match(self, layer1, layer2, direction):
        ...
        for (k1, item1), (k2, item2) in zip(layer1.items(), layer2.items()):
            if item1['pos'][direction] != item2['pos'][direction]:
                ...
                tmp1 = [v for i, v in enumerate(item1['pos']) if i != direction]
                tmp2 = [v for i, v in enumerate(item2['pos']) if i != direction]
