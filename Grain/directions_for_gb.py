...
LN = smart_structure_read(input_geo_file = "/Users/ndavydov/Documents/Skoltech/Science/cif's/POSCAR_LiNiO2_r3m_su_4uiAg_100_end_from_dima_aks")
LNconv = LN.copy()
...
print(min(np.linalg.norm( np.cross(LN.rprimd[0], LN.rprimd[1])  ), np.linalg.norm( np.cross(LN.rprimd[0], LN.rprimd[2])  ), np.linalg.norm( np.cross(LN.rprimd[1], LN.rprimd[2])  )))
